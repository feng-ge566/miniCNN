...
from quan.quant_sparse_layers import Quanconv2_feng,QuanLinear_feng
...
class VGG(nn.Module):
    ...
    def __init__(
        ...
        self.classifier = nn.Sequential(
            ...
            QuanLinear_feng(in_features=64 * 8 * 8, out_features=64, 
                             sparsity=vs[0], 
                             scale=scale[0],
            ...
            QuanLinear_feng(in_features=64, out_features=num_classes, 
                             sparsity=vs[1],
                             scale=scale[1],
            ...
            # QuanLinear_feng(in_features=128, out_features=64, sparsity=vs[2],
            #                  TIN=tin[2], group_size=vg[2], 
            #                  weight_bit_width=vb[2],activation_bit_width=va[2]),
            # nn.ReLU(True),
            # nn.Dropout(),

            # QuanLinear_feng(in_features=64, out_features=num_classes, sparsity=vs[2],
            #                  TIN=tin[2], group_size=vg[2], 
            #                  weight_bit_width=vb[3],activation_bit_width=va[3]),
    ...
    def init_weight(self):
        for m in self.modules():
            if isinstance(m, QuanConv2d) or isinstance(m,QuanLinear) or isinstance(m,QuanLinear_feng) or isinstance(m,Quanconv2_feng):
                m._init_weight()
    ...
    def calculate_complexity(self):
        total = 0
        for m in self.modules():
            if isinstance(m,QuanLinear_feng) or isinstance(m,Quanconv2_QuanLinear_feng):
...
def make_layers_vgg16(cfg: List[Union[str, int]], batch_norm: bool = False) -> nn.Sequential:
    ...
    for v in cfg:
        if v == 'M':
            ...
            if(vww==1 and vaw==1):
                conv2d = SiMAN_BinarizeConv2d(in_channels, vo, kernel_size=3, stride=strd, padding=1, bias=False)
                ...
                conv2d = Quanconv2_feng(in_channels = in_channels, out_channels=vo, 
                                      weight_bit_width =vww,
                                      activation_bit_width=vaw,
                                      scale=scale,
                                      sparsity = vs, TIN=vTin, group_size=vg,
                                      kernel_size=3, padding=1,stride=strd)
...
kbs_cfgs: Dict[str, List[Union[str, int]]] = {
    ...
    'D': [[ 64,8,8,0.00,16,1], [ 64,4,4,0.00,16,1], 'M', #【Cin, Weight_bit_width, Activation_bit_width, sparsity, Tin, Group】
          ...
          [256,4,4,0.00,16,1], [256,2,2,0.00,16,1], [256,4,4,0.00,16,1], 'M', 
          [512,4,4,0.00,16,1], [512,1,1,0.00,16,1], [512,4,4,0.00,16,1], 'M', 
          [512,4,4,0.00,16,1], [512,2,2,0.00,16,1], [512,4,4,0.00,16,1], 'M'],
    'E': [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 256, 'M', 512, 512, 512, 512, 'M', 512, 512, 512, 512, 'M'],
    'F': [[ 64,8,8,0.00,16,1,1,-3], [64,8,8,0.00,16,1,1,-3], [128,8,8,0.00,16,1,2,-3], [64,8,8,0.00,16,1,1,-3], [64,8,8,0.00,16,1,2,-3]],
}
...
def vgg16(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> VGG:
    ...
    """
    return _vgg16('vgg16', 'F', False, pretrained, progress, **kwargs)
